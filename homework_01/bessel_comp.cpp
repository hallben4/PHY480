...
main ()
{
  ...
  ofstream my_out ("bessel_comp.dat");
  ...
  for (double x = xmin; x <= xmax; x += step)
    {
      ...
      double ans = gsl_sf_bessel_jl (order, x);
      double diff = fabs(ans_down-ans_up)/(fabs(ans_down)+fabs(ans_up));

      my_out << fixed << setprecision (6) << setw (8)
    << log10(x) << " "
  ...
  cout << "data stored in bessel_comp.dat." << endl;
