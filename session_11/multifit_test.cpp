...
main (void)
{
  ...
  // allocate space for a covariance matrix of size p by p and Jacobian
  gsl_matrix *covariance_ptr = gsl_matrix_alloc (p, p);
  gsl_matrix *Jacobian_ptr = gsl_matrix_alloc (n,p);
  ...
  // Create file for real function data
  ofstream rout;
  rout.open("multi_fit_function");
  for (double t = 0; t < n; t++)
    {
      double A = fit (0, solver_ptr);
      double lambda = fit (1, solver_ptr);
      double b = fit (2, solver_ptr);
      double y_actual = A * exp(-lambda * t) + b;
        
      // print out the data to make sure it looks ok
      rout << "   " << t << "   " << y_actual << endl;
    };
