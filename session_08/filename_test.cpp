...
main (void)
{
  ...
  cout << "Look at " << my_filename_stream.str() 
    ...
    for (int j = 0; j <= 3; j++)
    {
        ostringstream my_filename_streams;  // declare a stringstream object
        
        // load the string
        my_filename_streams << "file" << j << ".out";
        // use .str() to convert to a string
        string my_filenames2 = my_filename_streams.str();

        cout << "The output filename is " << my_filenames2 << endl;
        
        ofstream my_files2;    // now for a filename
        my_files2.open (my_filenames2.c_str());  // use .c_str() to convert to a char*
        
        my_files2 << "This is file " << j << endl;
        
        my_files2.close();    // close the output file
